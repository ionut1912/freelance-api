...
public class TimeLogRepository : ITimeLogRepository
{
    ...
    public async Task CreateTimeLogAsync(CreateTimeLogCommand createTimeLogCommand, CancellationToken cancellationToken)
    {
        ...
        if (timeLogTask is null)
            throw new NotFoundException(
                $"{nameof(ProjectTasks)} with {nameof(ProjectTasks.Title)} : '{createTimeLogCommand.CreateTimeLogRequest.TaskTitle}' does not exist");

        var timeLog = createTimeLogCommand.CreateTimeLogRequest.Adapt<TimeLogs>();
        timeLog.TaskId = timeLogTask.Id;
        timeLog.TotalHours = createTimeLogCommand.CreateTimeLogRequest.EndTime.Hour -
                             createTimeLogCommand.CreateTimeLogRequest.StartTime.Hour;
    ...
    public async Task UpdateTimeLogAsync(UpdateTimeLogCommand updateTimeLogCommand, CancellationToken cancellationToken)
    {
        if (updateTimeLogCommand.UpdateTimeLogRequest.TaskTitle is null)
            throw new NotFoundException(
                $"{nameof(ProjectTasks)} with {nameof(ProjectTasks.Title)} : {updateTimeLogCommand.UpdateTimeLogRequest.TaskTitle} not found");
        ...
        if (timeLogTask is null)
            throw new NotFoundException(
                $"{nameof(ProjectTasks)} with {nameof(ProjectTasks.Title)} : '{updateTimeLogCommand.UpdateTimeLogRequest.TaskTitle}' does not exist");
        var timeLogToUpdate = await _timeLogsRepository.Query()
            .Where(x => x.Id == updateTimeLogCommand.Id)
            .FirstOrDefaultAsync(cancellationToken);
        if (timeLogToUpdate is null)
            throw new NotFoundException(
                $"{nameof(TimeLogs)} with {nameof(TimeLogs.Id)} : '{updateTimeLogCommand.Id}' does not exist");

        updateTimeLogCommand.UpdateTimeLogRequest.Adapt(timeLogToUpdate);
    ...
    public async Task DeleteTimeLogAsync(DeleteTimeLogCommand deleteTimeLogCommand, CancellationToken cancellationToken)
    {
        ...
        if (timeLogToRemove is null)
            throw new NotFoundException(
                $"{nameof(TimeLogs)} with {nameof(TimeLogs.Id)} : '{deleteTimeLogCommand.Id}' does not exist");
        _timeLogsRepository.Delete(timeLogToRemove);
    }

    public async Task<TimeLogDto> GetTimeLogByIdAsync(GetTimeLogByIdQuery getTimeLogByIdQuery,
        CancellationToken cancellationToken)
    {
        ...
        if (timeLog is null)
            throw new NotFoundException(
                $"{nameof(TimeLogs)} with {nameof(TimeLogs.Id)} : '{getTimeLogByIdQuery.Id}' does not exist");
        return timeLog.Adapt<TimeLogDto>();
    }

    public async Task<PaginatedList<TimeLogDto>> GetTimeLogsAsync(GetTimeLogsQuery getTimeLogsQuery,
        CancellationToken cancellationToken)
    {
        ...
        return new PaginatedList<TimeLogDto>(items, count, getTimeLogsQuery.PaginationParams.PageNumber,
            getTimeLogsQuery.PaginationParams.PageSize);
    }
}