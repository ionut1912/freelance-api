...
public class ClientProfileService:IClientProfileRepository
{
    ...
    public async Task<ClientProfile> GetClientProfileByIdAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        return clientProfile == null ? throw new ProfileNotFoundException($"Profile with id {id} not found") : clientProfile;
    }
    ...
    public async Task<ClientProfile> GetLoggedInFreelancerProfileAsync(Guid accountId, CancellationToken cancellationToken)
    {
        var clientProfile= await _context.ClientProfiles
            .AsNoTracking()
            .FirstOrDefaultAsync(x=>x.AccountId == accountId, cancellationToken);
        return clientProfile == null ? throw new ProfileNotFoundException($"Profile with AccountId {accountId} not found") : clientProfile;
    }
}